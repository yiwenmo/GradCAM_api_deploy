...
class yolov5_heatmap:
    ...
    def draw_detections(self, boxes, colors, names, img):
    ...
    def __call__(self, img_path, save_path, output_filename='output'):
        ...
        for i in trange(self.genCAMNum):
            ...
            # boxes_to_draw = [post_boxes[i] for i in range(self.genCAMNum) if post_result[i][0] >= self.conf_threshold]
            # colors_to_draw = [self.colors[int(post_result[i, 1:].argmax())] for i in range(self.genCAMNum) if post_result[i][0] >= self.conf_threshold]
            # names_to_draw = [f'{self.model_names[int(post_result[i, 1:].argmax())]} {post_result[i][0]-0.01:.2f}' for i in range(self.genCAMNum) if post_result[i][0] >= self.conf_threshold]            
            ...
            # cam_image = self.draw_detections(boxes_to_draw, colors_to_draw, names_to_draw, cam_image)
...
def get_params():
    params = {
        'weight': 'models/best.pt',  # 訓練權重檔案.pt
        'cfg': 'models/yolov5s_arcade.yaml',  # yaml存取模型架構
